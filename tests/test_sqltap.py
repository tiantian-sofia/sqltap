...
class TestSQLTap(object):
    ...
    def setup_method(self):
        ...
        # track created sessions so teardown can close them explicitly;
        # sessions that are GC'd while still holding a checked-out
        # connection make SQLAlchemy emit a warning from a weakref
        # finalizer, which is an error under the warnings filter
        self._sessions = []
        session_factory = sessionmaker(bind=self.engine)

        def tracked_session(*args, **kwargs):
            sess = session_factory(*args, **kwargs)
            self._sessions.append(sess)
            return sess

        self.Session = tracked_session
